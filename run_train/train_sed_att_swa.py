...
swa_scheduler = SWALR(optimizer,swa_lr=CFG.lr/20)
swa_start = 15
...
for i in range(CFG.epochs):
    ...
    if cmAP_pad5_score> best_cmAP:
        ...
        torch.save(swa_model.state_dict(),CFG.log_dir+current_time+'/saved_model.pt')
    else:
        logger.info(f'epoch:{i} val_cmAP did not improved from {best_cmAP}, now val_cmAP is {cmAP_pad5_score}...')
torch.save(swa_model.state_dict(),CFG.log_dir+current_time+'/saved_model_lastepoch.pt')
