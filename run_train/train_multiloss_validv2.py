...
CFG.train_path = '/root/projects/BirdClef2025/BirdCLEF2023-30th-place-solution-master/usefulFunc/train_duration.csv'
df_all = pd.read_csv(CFG.train_path)
# 从df_train中duration<15s的sample中随机采样1000个作为valid
df_durationle15 = df_all[df_all['duration']<15]
df_duration15_valid = df_durationle15.sample(n=630,random_state=42)
df_durationle15 = df_durationle15.drop(df_duration15_valid.index)
df_train = df_all[df_all['duration']>=15]
df_train = pd.concat((df_train,df_durationle15))
df_valid = df_duration15_valid
birds_val_col = list(pd.get_dummies(df_valid['primary_label']).columns)

external_train = pd.read_csv('/root/projects/BirdClef2025/data/external_trainv2.csv')
...
classes_auc_dict = dict(zip(birds_val_col,[[] for _ in birds_val_col]))
...
for i in range(CFG.epochs):
    ...
    scored_cols = np.where(gtall_numpy.sum(axis=0)!=0)[0]
    scored_cols = [birds[i] for i in scored_cols]
    gt_df = gt_df[scored_cols]
    pred_df = pred_df[scored_cols]
