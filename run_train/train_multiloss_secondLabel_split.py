...
from model import BirdClefModel,BirdClefSEDModel,BirdClefSEDAttModel,BirdClefSEDAttModelSplit
...
from utils import *
from losses import SmoothBCEFocalLoss,MultiLoss,MultiLossWeighting,BCELoss,Splitloss
...
low_numclasses_birds = list(df_train['primary_label'].value_counts()[(df_train['primary_label'].value_counts()<15)].index)
...
trainloader = DataLoader(train_set,batch_size=CFG.batch_size,num_workers=16,pin_memory=True)
valloader = DataLoader(val_set,batch_size=CFG.batch_size,shuffle=False,num_workers=16,pin_memory=True)

# loss
# criterion = torch.nn.BCEWithLogitsLoss()
loss_ = Splitloss(high_numclasses_birds_index,low_numclasses_birds_index)
# loss_ = MultiLossWeighting(smoothing=CFG.smoothing_factor)

# model
model = BirdClefSEDAttModelSplit(high_numclasses_birds_index,low_numclasses_birds_index,CFG.model,num_classes=CFG.num_classes,pretrained=CFG.pretrained).cuda()
...
classes_auc_dict = dict(zip(birds,[[] for _ in birds]))
classes_auc_dict['avg'] = []
...
for i in range(CFG.epochs):
    ...
    for idx,(audios,labels,weights) in enumerate(valloader):
        ...
        with torch.no_grad():
            ...
            pred1,pred2 = pred
            pred1 = (pred1['clipwise_output'] + pred1['maxframewise_output'])/2
            pred2 = (pred2['clipwise_output'] + pred2['maxframewise_output'])/2
            pred = torch.zeros_like(labels).to(labels.device).to(torch.float32)
            classes1_index = torch.tensor(high_numclasses_birds_index).to(labels.device)
            classes2_index = torch.tensor(low_numclasses_birds_index).to(labels.device)
            pred[:,classes1_index] = pred1
            pred[:,classes2_index] = pred2
    ...
    gt_df = gt_df[(gt_df.sum()>0).index]
    pred_df = pred_df[(gt_df.sum()>0).index]
    for col in list(gt_df.columns):
        classes_auc_dict[col].append(sklearn.metrics.roc_auc_score(gt_df[col],pred_df[col],))
    classes_auc_dict['avg'].append(sklearn.metrics.roc_auc_score(np.int16(scored_gt),scored_pred,))
    pd.DataFrame(classes_auc_dict).to_csv(CFG.log_dir+current_time+'/valid.csv',index=False)
