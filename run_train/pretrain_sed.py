...
from losses import SmoothBCEFocalLoss,MultiLoss,BCELoss
from utils import *
    ...
    for idx,(audios,labels) in enumerate(trainloader):
        ...
        with amp.autocast(enabled=True):
            if data_label_list[-1]:
                ...
                loss = mixup_criterion(loss_,pred,label_a,label_b,data_label_list[3],classes_weight=None)
            else:
                images = data_label_list[0]
                pred = model(images)
                loss = loss_(pred,data_label_list[1],weights=None)
